...
const TOUR_CROSS_LABELS = {
  tourBucket: "Kỳ",
  tourMonth: "Tháng (matrix)",
  weekday: "Thứ",
  buoi: "Buổi",
  tienDo: "Tiến độ",
  truong: "Trường",
  unscheduledMonth: "Tháng (chưa xác định lịch)",
};
...
export function B2BPage() {
  ...
  /** Quy bucket theo unit đang chọn: có ngày cụ thể -> bucketOf như bình thường; chỉ có Tháng
   *  (chưa chốt ngày) -> vẫn gộp được nếu unit thô hơn ngày (Tháng/Quý/Năm), còn Ngày/Tuần/Thứ
   *  thì KHÔNG suy diễn được ngày cụ thể -> null (rơi vào cột "Chưa xác định lịch tour"). */
  function tourBucketOf(r: TourRow, unit: TimeUnit): string | null {
    if (r.hasDate && r.date) return bucketOf(r.date, unit);
    if (unit === "month") return r.month;
    if (unit === "quarter") {
      const [y, m] = r.month.split("-").map(Number);
      return `${y}-Q${Math.ceil((m ?? 1) / 3)}`;
    }
    if (unit === "year") return r.month.split("-")[0] ?? null;
    return null;
  }
  ...
  const tourData = useMemo(() => {
    ...
    const rows = tourRows.filter(
      (r) =>
        ...
        matchSel(tourBucketOf(r, unit), tourSel["tourBucket"]) &&
        matchSel(r.hasDate ? null : r.month, tourSel["unscheduledMonth"]) &&
        matchSel(r.month, tourSel["tourMonth"]),
    );

    const totalTours = rows.length;
    const totalStudents = rows.reduce((s, r) => s + r.students, 0);
    const totalRevenue = rows.reduce((s, r) => s + r.revenue, 0);
    const totalSchools = new Set(rows.map((r) => r.schoolName)).size;

    /* Số lượng lượt tour theo thời gian, chia theo Buổi — dùng chung đơn vị thời gian với thanh
     * chọn nhanh phía trên B2B (filters.timeUnit). Cột cuối "Chưa xác định lịch tour": các tour
     * chỉ mới biết Tháng, chưa chốt ngày cụ thể, nên không đặt được vào đúng Ngày/Tuần/Thứ khi xem
     * ở granularity mịn hơn Tháng. */
    const bucketKeys = sortBuckets(
      Array.from(new Set(rows.map((r) => tourBucketOf(r, unit)).filter((b): b is string => b !== null))),
      unit,
    );
    const unscheduledRows = rows.filter((r) => tourBucketOf(r, unit) === null);
    const timeByBuoi = [
      ...bucketKeys.map((b) => {
        const bRows = rows.filter((r) => tourBucketOf(r, unit) === b);
        const entry: Record<string, number | string> = { bucket: b, name: bucketLabel(b, unit) };
        for (const buoi of BUOI_ORDER) entry[buoi] = bRows.filter((r) => r.buoi === buoi).length;
        entry.total = bRows.length;
        return entry;
      }),
      ...(unscheduledRows.length > 0
        ? [
            (() => {
              const entry: Record<string, number | string> = { bucket: UNSCHEDULED, name: "Chưa xác định lịch tour" };
              for (const buoi of BUOI_ORDER) entry[buoi] = unscheduledRows.filter((r) => r.buoi === buoi).length;
              entry.total = unscheduledRows.length;
              return entry;
            })(),
          ]
        : []),
    ...
    /* Số lượng học sinh Dự kiến (SL HS) vs Thực tế (SL TT) theo thời gian — chỉ cộng SL TT các dòng
     * đã có (không suy diễn 0 cho dòng chưa cập nhật thực tế). */
    const hsCompareTimeline = bucketKeys.map((b) => {
      const bRows = rows.filter((r) => tourBucketOf(r, unit) === b);
      const duKien = bRows.reduce((s, r) => s + r.students, 0);
      const thucTe = bRows.reduce((s, r) => s + (r.actualStudents ?? 0), 0);
      return {
        bucket: b,
        name: bucketLabel(b, unit),
        "SL HS dự kiến": duKien,
        "SL TT thực tế": thucTe,
        "Chênh lệch": duKien - thucTe,
      };
    });
    ...
    /* Top trường theo Doanh thu dự kiến / theo Số lượng học sinh dự kiến. */
    const schoolMap = new Map<string, { revenue: number; students: number; count: number }>();
    for (const r of rows) {
      const cur = schoolMap.get(r.schoolName) ?? { revenue: 0, students: 0, count: 0 };
      cur.revenue += r.revenue;
      cur.students += r.students;
      cur.count += 1;
    ...
    const topSchools = Array.from(schoolMap.entries())
      ...
      .sort((a, b) => b.value - a.value)
      .slice(0, 10);
    const topSchoolsByStudents = Array.from(schoolMap.entries())
      .map(([name, v]) => ({
        name,
        value: v.students,
        revenue: v.revenue,
        count: v.count,
        share: totalStudents ? (v.students / totalStudents) * 100 : 0,
      }))
      .sort((a, b) => b.value - a.value)
      .slice(0, 10);

    /* Matrix Tháng, Tuần & Thứ, Buổi — value = SL HS. "Full ngày" tính vào cả Sáng lẫn Chiều
     * (chiếm trọn slot); "Không xác định" (buổi) không quy được vào Sáng/Chiều nên không tính vào
     * matrix này. Cột "Tour chưa xác định lịch" tách 2 số: Số tour và Số HS tương ứng — tour chỉ
     * biết Tháng, chưa chốt ngày -> không đặt được vào ô Thứ nào, vẫn tính đúng vào hàng Tháng. */
    const dateRows = rows.filter((r): r is TourRow & { date: string } => r.hasDate && r.date !== null);
    const undatedRows = rows.filter((r) => !r.hasDate);
    const months = sortBuckets(Array.from(new Set(rows.map((r) => r.month))), "month");
    ...
    const tourMatrix = months.map((mk) => {
      const mRows = dateRows.filter((r) => bucketOf(r.date, "month") === mk);
      const mUndated = undatedRows.filter((r) => r.month === mk);
      ...
      const unscheduledCount = mUndated.length;
      const unscheduledStudents = mUndated.reduce((s, r) => s + r.students, 0);
      return {
        monthKey: mk,
        ...
        unscheduledCount,
        unscheduledStudents,
        total: mRows.reduce((s, r) => s + r.students, 0) + unscheduledStudents,
      };
    });
    const tourMatrixMax = Math.max(
      ...
      ...tourMatrix.map((m) => m.unscheduledStudents),
    );
  ...
  }, [tourDataVersion, tourSel, filters.timeUnit]);
  ...
  const sortedTourRows = [...tourData.rows].sort((a, b) => (a.date ?? `${a.month}-99`).localeCompare(b.date ?? `${b.month}-99`));
  ...
  return (
    ...
    >
      ...
      <ImportDataBar
        ...
        description='Đủ 20 cột gốc của sheet "Lịch tour B2B". Cột "Ngày tham quan" nhập theo dạng dd/MM/yyyy (vd 31/12/2025) — đúng định dạng khi tải từ Google Sheets. Sheet gốc có 2 cột trùng tên "Note" nên cột cuối được đặt là "Note 2" để phân biệt — vị trí và toàn bộ cột khác giữ nguyên.'
      ...
      {tourView === "table" ? (
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <KpiCard label="Số trường có tour dự kiến" numeric={tourData.totalSchools} format={(n) => formatNumber(Math.round(n))} unit="trường" change={null} icon={<Building2 className="size-4" />} />
          </div>

          <div className="grid gap-4 xl:grid-cols-2">
            <Panel
              title="Số lượng lượt tour dự kiến theo thời gian"
              subtitle="Chia theo Buổi: Sáng (trước 12:00) / Chiều (từ 12:01) / Full ngày (>4 tiếng). Đơn vị thời gian dùng chung với thanh 'Đơn vị thời gian' phía trên B2B. Tour chỉ mới biết Tháng (chưa chốt ngày) vẫn tính được khi xem theo Tháng/Quý/Năm; xem theo Ngày/Tuần thì gộp vào cột cuối."
              code="CH-TOUR-01"
            ...
            >
              <ResponsiveContainer width="100%" height={320}>
                <ComposedChart
                  ...
                  margin={{ top: 16, right: 8, left: 0, bottom: 0 }}
                  onClick={(e: { activePayload?: { payload?: { bucket?: string } }[] }) => {
                    const bucket = e?.activePayload?.[0]?.payload?.bucket;
                    if (bucket && bucket !== UNSCHEDULED) tourToggle("tourBucket", bucket);
                  }}
                  style={{ cursor: "pointer" }}
                >
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_COLORS.grid} vertical={false} />
          ...
          <Panel
            title="Số lượng học sinh Dự kiến vs Thực tế"
            subtitle="SL HS (dự kiến) so với SL TT (thực tế, chỉ tính dòng đã cập nhật) — bấm cột để lọc chéo theo kỳ"
            code="CH-TOUR-05"
            isEmpty={tourData.hsCompareTimeline.length === 0}
          >
            <ResponsiveContainer width="100%" height={320}>
              <BarChart
                data={tourData.hsCompareTimeline}
                margin={{ top: 16, right: 8, left: 0, bottom: 0 }}
                onClick={(e: { activePayload?: { payload?: { bucket?: string } }[] }) =>
              ...
              >
                ...
                <Tooltip
                  ...
                  content={({ active, payload, label }) => {
                    ...
                    return (
                      <TooltipBox label={String(label)}>
                        <TooltipRow color={CHART_COLORS.dark} name="SL HS dự kiến" value={Number(row["SL HS dự kiến"] ?? 0)} unit="học sinh" />
                        <TooltipRow color={CHART_COLORS.primary} name="SL TT thực tế" value={Number(row["SL TT thực tế"] ?? 0)} unit="học sinh" />
                        <TooltipRow color={CHART_COLORS.danger} name="Chênh lệch (Dự kiến − Thực tế)" value={Number(row["Chênh lệch"] ?? 0)} unit="học sinh" />
                      </TooltipBox>
                    );
                  }}
                />
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Bar dataKey="SL HS dự kiến" fill={CHART_COLORS.dark} radius={[4, 4, 0, 0]} cursor="pointer" />
                <Bar dataKey="SL TT thực tế" fill={CHART_COLORS.primary} radius={[4, 4, 0, 0]} cursor="pointer" />
              </BarChart>
            </ResponsiveContainer>
          </Panel>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            ...
            <Panel
              title="Top trường theo Số lượng học sinh dự kiến"
              subtitle="Bấm vào cột để lọc chéo toàn khu vực Lịch tour B2B"
              code="CH-TOUR-03B"
              isEmpty={tourData.topSchoolsByStudents.length === 0}
            >
              <ResponsiveContainer width="100%" height={340}>
                <BarChart data={tourData.topSchoolsByStudents} layout="vertical" margin={{ top: 8, right: 60, left: 8, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={CHART_COLORS.grid} horizontal={false} />
                  ...
                  <Tooltip
                    ...
                    content={({ active, payload }) => {
                      if (!active || !payload?.length) return null;
                      const row = payload[0]?.payload as { name: string; value: number; revenue: number; count: number; share: number };
                      return (
                        <TooltipBox label={row.name}>
                          <TooltipRow name="Số học sinh dự kiến" value={row.value} unit="học sinh" share={row.share} />
                          <TooltipRow name="Số lượt tour" value={row.count} unit="lượt" />
                          <TooltipRow name="Doanh thu dự kiến" value={row.revenue} />
                        </TooltipBox>
                      );
                    }}
                  />
                  <Bar
                    ...
                    onClick={(d: { name?: string }) => tourToggle("truong", d?.name)}
                  >
                    {tourData.topSchoolsByStudents.map((d) => (
                      <Cell key={d.name} fill={cellFill(d.name, tourSel["truong"], CHART_COLORS.primary)} />
                    ))}
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-muted-foreground">
                    ...
                    <th colSpan={2} className="border-l border-border/60 px-2 py-1 text-center font-medium">
                      Tour chưa xác định lịch
                    </th>
                  ...
                  <tr className="text-muted-foreground">
                    ...
                    <th className="border-l border-border/60 px-1.5 py-1 text-right font-normal">Số tour</th>
                    <th className="px-1.5 py-1 text-right font-normal">Số HS</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <Fragment key={m.monthKey}>
                        <tr
                          ...
                          onClick={() => {
                            ...
                            tourToggle("tourMonth", m.monthKey);
                          }}
                        >
                          <td className="px-2 py-1 whitespace-nowrap">
                            <span className="inline-block w-3 text-muted-foreground">{open ? "▾" : "▸"}</span> {m.month}
                          ...
                          <td
                            title={`${m.month} · Số tour chưa xác định lịch: ${formatNumber(m.unscheduledCount)}`}
                            className="cursor-pointer border-l border-border/60 px-1.5 py-1 text-right tabular-nums text-muted-foreground hover:underline"
                            style={{ background: tint(m.unscheduledStudents) }}
                            onClick={(e) => {
                              e.stopPropagation();
                              tourToggle("unscheduledMonth", m.monthKey);
                            }}
                          >
                            {m.unscheduledCount > 0 ? formatNumber(m.unscheduledCount) : "—"}
                          </td>
                          <td
                            title={`${m.month} · Số HS (tour chưa xác định lịch): ${formatNumber(m.unscheduledStudents)} HS`}
                            className="cursor-pointer px-1.5 py-1 text-right tabular-nums text-muted-foreground hover:underline"
                            style={{ background: tint(m.unscheduledStudents) }}
                            onClick={(e) => {
                              e.stopPropagation();
                              tourToggle("unscheduledMonth", m.monthKey);
                          ...
                          >
                            {m.unscheduledStudents > 0 ? formatNumber(m.unscheduledStudents) : "—"}
                          </td>
                        ...
                        {open &&
                          m.weeks.map((w) => (
                            <tr key={`${m.monthKey}-${w.weekLabel}`} className="text-muted-foreground">
                              ...
                              <td className="border-l border-border/60 px-1.5 py-1 text-right tabular-nums text-muted-foreground">—</td>
                              <td className="px-1.5 py-1 text-right tabular-nums text-muted-foreground">—</td>
          ...
          <FactTable
            title="Bảng chi tiết lịch tour"
            subtitle="Toàn bộ cột gốc trong sheet Lịch tour B2B — chọn cột hiển thị và xuất dữ liệu"
            columns={tourColumns}
            rows={sortedTourRows}
            fileName="b2b-lich-tour-chi-tiet"
          />
        </>
      )}
