import { AlertTriangle, CalendarDays, ChevronLeft, ChevronRight, LayoutGrid, Lightbulb } from "lucide-react";
...
import { EMPTY_TEXT, Panel } from "@/features/stemtown/components/panel";
...
export function TourPage() {
  ...
  return (
    <DashboardShell
      title="Lịch tour B2B"
      description="Dựa trên lịch tham quan đã xếp (kể cả chưa nghiệm thu). Nguồn dữ liệu riêng, doanh thu ở đây là doanh thu dự kiến, không tính vào doanh thu B2B nghiệm thu."
    >
...
function DeltaText({ d, suffix }: { d: number | null; suffix?: string }) {
  const dir = direction(d);
  if (dir === null) return <span className="text-muted-foreground">Chưa so được</span>;
  const cls = dir === "up" ? "text-[var(--success)]" : dir === "down" ? "text-destructive" : "text-muted-foreground";
  const icon = dir === "up" ? "▲" : dir === "down" ? "▼" : "●";
  return (
    <span className={cls}>
      {icon} {formatNumber(Math.abs(Math.round((d as number) * 100)))}%{suffix ? <span className="text-muted-foreground"> {suffix}</span> : null}
    </span>
  );
}

function FormulaCard({ color, label, value, unit, d, hint }: { color: string; label: string; value: string; unit: string; d: number | null; hint: string }) {
  return (
    <div className="rounded-xl border border-border bg-card p-4 shadow-xs" style={{ borderTop: `4px solid ${color}` }} title={hint}>
      <p className="flex items-center gap-1.5 text-sm font-medium text-muted-foreground">
        <span className="size-2.5 rounded-sm" style={{ background: color }} aria-hidden />
        {label}
      </p>
      <p className="mt-2 flex items-baseline gap-1 text-2xl font-semibold tracking-tight text-foreground">
        {value}
...
function Op({ children }: { children: ReactNode }) {
  return (
    <div className="hidden items-center justify-center text-xl text-muted-foreground lg:flex" aria-hidden>
      {children}
    </div>
  );
}
...
function TourOverview({ items, today }: { items: TourItem[]; today: string }) {
  ...
  const sub = useMemo(() => {
    const withTT = list.filter((r) => r.actualStudents !== null);
    const plan = withTT.reduce((a, r) => a + r.students, 0);
    const act = withTT.reduce((a, r) => a + (r.actualStudents ?? 0), 0);
    const load: Record<string, number> = {};
    list.forEach((r) => {
      if (r.date) load[r.date] = (load[r.date] ?? 0) + r.students;
    });
    return {
      withTT: withTT.length,
      plan,
      act,
      days: Object.keys(load).length,
      over: Object.values(load).filter((v) => v > TOUR_RULES.capacityWarn).length,
      noStatus: list.filter((r) => r.date && r.date < today && r.status === "Chưa xác định").length,
    };
  }, [list, today]);
  ...
  const dq = useMemo(() => {
    ...
    return [
      ...
      `Ngưỡng cảnh báo ${TOUR_RULES.capacityWarn} HS là tạm thời (ước tính ${TOUR_RULES.capacityRange}), áp cho cả từng tour và tổng HS trong ngày.`,
  ...
  return (
    <div className="space-y-4">
      ...
      <StepHeader n={1} title="Tháng này thế nào?" subtitle="Doanh thu dự kiến = Số tour × HS mỗi tour × Giá mỗi HS. Màu của 4 chỉ số giữ nguyên ở mọi chart." />
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_20px_1fr_20px_1fr_20px_1fr]">
        <FormulaCard color={MC.tour} label="Số tour" value={formatNumber(cur.n)} unit="tour" d={delta(cur.n, base.n)} hint="Số lượt trường/ngày trên lịch, không tính khách mời, sự kiện và tour chưa có ngày." />
        <Op>×</Op>
        <FormulaCard color={MC.size} label="HS mỗi tour" value={cur.size === null ? "—" : formatNumber(Math.round(cur.size))} unit="HS" d={delta(cur.size, base.size)} hint="Tổng HS dự kiến chia số tour." />
        <Op>×</Op>
        <FormulaCard color={MC.price} label="Giá mỗi HS" value={cur.price === null ? "—" : formatNumber(Math.round(cur.price / 1000))} unit="k" d={delta(cur.price, base.price)} hint="Doanh thu dự kiến chia tổng HS dự kiến (bình quân có trọng số)." />
        <Op>=</Op>
        <FormulaCard color={MC.dt} label="Doanh thu dự kiến" value={cur.n ? formatNumber(cur.dt / 1_000_000, cur.dt < 100_000_000 ? 1 : 0) : "—"} unit="tr" d={delta(cur.dt, base.dt)} hint="Tổng cột Doanh thu dự kiến trên lịch. Không phải doanh thu nghiệm thu." />
      </div>
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard label="Tổng HS dự kiến" value={cur.n ? formatNumber(cur.hs) : "—"} unit="HS" change={delta(cur.hs, base.hs) === null ? null : (delta(cur.hs, base.hs) as number) * 100} subtitle={`So với ${base.label}`} />
        <KpiCard
          label="HS thực tế (cột SL TT)"
          value={sub.withTT ? formatNumber(sub.act) : "Chưa có"}
          unit={sub.withTT ? "HS" : undefined}
          change={null}
          subtitle={sub.withTT ? `Đạt ${formatNumber(Math.round((sub.act / sub.plan) * 100))}% dự kiến, trên ${sub.withTT}/${list.length} tour có SL TT` : "Chưa có tour nào nhập SL TT"}
        />
        <KpiCard
          label={`Ngày trên ${TOUR_RULES.capacityWarn} HS`}
          value={formatNumber(sub.over)}
          unit={`/ ${sub.days} ngày có tour`}
          change={null}
          valueClassName={sub.over ? "text-[var(--brand-accent)]" : undefined}
          subtitle={`Ngưỡng tạm thời ${TOUR_RULES.capacityRange} HS`}
        />
        <KpiCard
          label="Tour chưa cập nhật tiến độ"
          value={formatNumber(sub.noStatus)}
          unit="tour"
          change={null}
          valueClassName={sub.noStatus ? "text-[var(--brand-accent)]" : undefined}
          subtitle="Đã qua ngày tham quan"
        />
      </div>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={sizeWin} margin={{ top: 8, right: 8, left: -10, bottom: 0 }}>
              ...
              {TOUR_RULES.sizeBins.map((b, i) => (
                <Bar key={b.label} dataKey={b.label} stackId="s" fill={i === 3 ? WARN : mix(CHART_COLORS.primary, [30, 60, 100][i] as number)} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        </Panel>
...
function TourSchedule({ items, today, active }: { items: TourItem[]; today: string; active: boolean }) {
  ...
  const sales = useMemo(() => Array.from(new Set(items.filter((r) => r.kind === "tour" && r.sale).map((r) => r.sale as string))).sort(), [items]);
  const heatMonths = useMemo(() => Array.from(new Set([...dated.map((r) => r.month), curMonth])).sort(), [dated, curMonth]);
  ...
  const [sale, setSale] = useState("");
  const [kinds, setKinds] = useState<Set<TourKind>>(() => new Set<TourKind>(["tour", "guest", "block"]));
  ...
  const wk = useMemo(() => {
    ...
    return {
      ...
      warnDays: weekInfos.filter((d) => d.load > TOUR_RULES.capacityWarn).length,
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-4">
        ...
        <fieldset className="grid gap-1 text-xs text-muted-foreground">
          <legend className="mb-1">Hiển thị</legend>
          <div className="flex flex-wrap gap-4">
            {(["tour", "guest", "block"] as const).map((k) => (
              <label key={k} className="inline-flex items-center gap-1.5 text-sm text-foreground">
                <input
                  type="checkbox"
                  checked={kinds.has(k)}
                  onChange={(e) =>
                ...
                />
                <span className={cn("inline-block h-3 w-4 rounded-sm", BAR_CLS[k])} aria-hidden />
                {KIND_LABEL[k]}
              </label>
            ))}
          </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-5">
        ...
        <KpiCard label={`Ngày >${TOUR_RULES.capacityWarn} HS`} value={formatNumber(wk.warnDays)} unit="ngày" change={null} valueClassName={wk.warnDays ? "text-[var(--brand-accent)]" : undefined} subtitle={`Ngưỡng tạm ${TOUR_RULES.capacityRange}`} />
        ...
        <section ref={hmCard} className="rounded-xl border border-border bg-card p-4 shadow-xs">
          ...
          <div className="my-2 flex flex-wrap items-center gap-3 text-[11px] text-muted-foreground">
            ...
            <span className="inline-flex items-center gap-1">
              <i className="inline-block size-3 rounded-sm" style={{ background: CHART_COLORS.primary, outline: `2px solid ${WARN}` }} />
              &gt;{TOUR_RULES.capacityWarn}
            </span>
            <span className="inline-flex items-center gap-1">
                          ...
                          return (
                            <tr key={`${m}-${w}`} data-w={w}>
                              ...
                              {Array.from({ length: 7 }, (_, i) => {
                                ...
                                const warn = inf.load > TOUR_RULES.capacityWarn;
                                ...
                                return (
                                  <td
                                    ...
                                    style={{
                                      ...
                                      outline: warn ? `2px solid ${WARN}` : undefined,
                                      outlineOffset: warn ? -2 : undefined,
                                      boxShadow: isFocus ? "inset 0 0 0 2px var(--foreground)" : undefined,
                                    }}
                                    title={`${WEEKDAYS[i]} ${dm(d)}: ${inf.tours} tour, ${formatNumber(inf.load)} HS${inf.peak > TOUR_RULES.capacityWarn && inf.peak < inf.load ? `, cao nhất ${formatNumber(inf.peak)} HS cùng lúc` : ""}${inf.hasOther ? ", có khách mời hoặc sự kiện" : ""}`}
          ...
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                {formatNumber(wk.tours)} tour, {formatNumber(wk.hs)} HS. Viền cam: đoàn &gt;{TOUR_RULES.capacityWarn} HS. Cạnh phải nét đứt: giờ kết thúc tự suy ra theo rule.
              </p>
            </div>
            ...
            return (
              <div key={d} className={cn("grid min-h-12 grid-cols-[112px_1fr] border-t border-border", d === focusDay && "bg-primary/5")}>
                <button type="button" className="py-1.5 pr-1.5 text-left text-xs leading-snug" onClick={() => setFocusDay(d)}>
                  ...
                  {inf.load > TOUR_RULES.capacityWarn && <span className="block font-semibold" style={{ color: WARN }}>⚠ Vượt {TOUR_RULES.capacityWarn} HS</span>}
                  {inf.peak > TOUR_RULES.capacityWarn && inf.peak < inf.load && <span className="block" style={{ color: WARN }}>Cao nhất {formatNumber(inf.peak)} HS cùng lúc</span>}
                </button>
                <div
                  className="relative"
                  style={{
                ...
                >
                  {bars.map((r) => {
                    ...
                    const big = r.kind === "tour" && r.students > TOUR_RULES.capacityWarn;
                    return (
                      <button
                        key={r.id}
                        type="button"
                        ...
                        style={{
                          ...
                          borderColor: big ? WARN : undefined,
                          borderWidth: big ? 2 : undefined,
                          borderLeftWidth: big ? 3 : undefined,
                ...
                tableRows.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-1.5 py-1.5 text-right tabular-nums" style={r.students > TOUR_RULES.capacityWarn ? { color: WARN, fontWeight: 600 } : undefined}>
