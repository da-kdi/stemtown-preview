...
const NUMERIC_COLUMNS = new Set(["SL HS", "Giá vé", "Doanh thu dự kiến"]);
...
export async function parseTourFile(file: File): Promise<ImportResult> {
  ...
  const required = ["Ngày tham quan", "Tên trường"];
...
export function downloadTourTemplate() {
  ...
  const body = tourSampleRaw
    .map((r) => TOUR_COLUMNS.map((c) => csvCell((r as Record<string, unknown>)[c] ?? "")).join(","))
    .join("\n");
