...
export type TourRow = {
  /** Ngày tham quan cụ thể (ISO yyyy-MM-dd); null nếu sheet chỉ có Tháng, chưa chốt ngày. */
  date: string | null;
  /** Tháng (yyyy-MM) — lấy từ ngày cụ thể nếu có, hoặc từ cột "Tháng" khi chưa có ngày cụ thể. */
  month: string;
  /** Có ngày tham quan cụ thể hay chỉ mới biết tháng. */
  hasDate: boolean;
  ...
  timeRaw: string | null;
  students: number;
  /** Cột "SL TT" (số lượng thực tế) — null nếu chưa nhập, không suy diễn = 0. */
  actualStudents: number | null;
  grade: string | null;
  anTrua: string | null;
  xe: string | null;
  note: string | null;
  soLuongGV: number | null;
  nguoiPhuTrach: string | null;
  thongTinLienHe: string | null;
  schoolName: string;
  region: string | null;
  sale: string | null;
  yeuCauYTe: string | null;
  duocSuDungHinhAnh: string | null;
...
function buoiOf(raw: unknown): TourRow["buoi"] {
  ...
  const matches = Array.from(s.matchAll(/(\d{1,2})[:h](\d{2})/g)).map((m) => Number(m[1]) * 60 + Number(m[2]));
  if (matches.length === 0) return "Không xác định";
  const start = matches[0] as number;
  // Có cả giờ bắt đầu & kết thúc -> tour dài hơn 4 tiếng tính là Full ngày, bất kể giờ bắt đầu.
  if (matches.length >= 2) {
    const end = matches[matches.length - 1] as number;
    const duration = end >= start ? end - start : end + 24 * 60 - start;
    if (duration > 4 * 60) return "Full ngày";
  }
...
/** Đúng 20 cột của sheet gốc "B2B - Trường cụ thể", giữ nguyên thứ tự & tên — kể cả 2 cột cùng
 *  tên "Note" (cột 7 và cột cuối). Vì object JS không cho 2 key trùng tên, cột Note thứ 2 được
 *  lưu nội bộ là "Note 2" (chỉ khác ở chỗ này, không đổi ở bất kỳ đâu khác/không rút gọn). */
...
/** Nhận diện ngày tham quan ở NHIỀU định dạng: ISO "yyyy-MM-dd" (data mẫu/JSON) hoặc "dd/MM/yyyy"
 *  (đúng định dạng Google Sheets Việt Nam xuất ra, và cũng là định dạng template CSV export) —
 *  kể cả khi dính kèm ghi chú như "21/07/2026\nBLOCK". Không suy diễn nếu không khớp mẫu nào. */
function parseTourDate(raw: unknown): string | null {
  const s = toStr(raw);
  if (!s) return null;
  ...
  const toIso = (dd: number, mm: number) => `${yyyy}-${String(mm).padStart(2, "0")}-${String(dd).padStart(2, "0")}`;
  // Ưu tiên dd/MM/yyyy (chuẩn Google Sheets VN + template). Nếu không hợp lệ ở thứ tự này nhưng
  // hợp lệ khi đảo lại (MM/dd/yyyy, vd file Excel từng tự hiển thị theo locale Mỹ) thì dùng đảo.
  const ddMmValid = a >= 1 && a <= 31 && b >= 1 && b <= 12;
  const mmDdValid = b >= 1 && b <= 31 && a >= 1 && a <= 12;
  if (ddMmValid) return toIso(a, b);
  if (mmDdValid) return toIso(b, a);
  return null;
}

export function mapTours(raw: Record<string, unknown>[]): TourRow[] {
  return raw
    .filter((r) => toStr(r["Tên trường"]))
    .map((r) => {
      const date = parseTourDate(r["Ngày tham quan"]);
      const month = date ? date.slice(0, 7) : monthOf(r["Tháng"]);
      return { date, month, hasDate: Boolean(date), r };
    })
    .filter((x): x is { date: string | null; month: string; hasDate: boolean; r: Record<string, unknown> } => Boolean(x.month))
    .map(({ date, month, hasDate, r }) => ({
      date,
      month,
      hasDate,
      ...
      anTrua: toStr(r["Ăn trưa"]),
      xe: toStr(r["Xe"]),
      note: toStr(r["Note"]),
      soLuongGV: (() => {
        const n = toNum(r["Số lượng GV"]);
        return n || toStr(r["Số lượng GV"]) ? n : null;
      })(),
      nguoiPhuTrach: toStr(r["Người phụ trách"]),
      thongTinLienHe: toStr(r["Thông tin liên hệ"]),
      schoolName: toStr(r["Tên trường"]) ?? "Không xác định",
      region: toStr(r["Khu vực"]),
      sale: toStr(r["Sale"]),
      yeuCauYTe: toStr(r["Yêu cầu y tế"]),
      duocSuDungHinhAnh: toStr(r["Được sử dụng hình ảnh"]),
...
export function filterTours(from: string, to: string): TourRow[] {
  return tourRows.filter((r) => r.date !== null && r.date >= from && r.date <= to);
}
