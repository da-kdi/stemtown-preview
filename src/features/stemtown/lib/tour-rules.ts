...
export function classifyTour(_r: TourRow): TourKind {
  return "tour";
}
...
/** Năm tài chính chứa ngày d: bắt đầu 1/7, kết thúc 30/6 năm sau. Trả về năm bắt đầu (vd 2025 cho FY 1/7/2025–30/6/2026). */
function fiscalYearStart(d: string): number {
  const [y, m] = d.split("-").map(Number) as [number, number];
  return m >= 7 ? y : y - 1;
}
const fyRange = (startYear: number): DateRange => ({ from: `${startYear}-07-01`, to: `${startYear + 1}-06-30` });
/** Quý tài chính 1–4 (Q1: T7–T9, Q2: T10–T12, Q3: T1–T3, Q4: T4–T6) chứa ngày d. */
function fiscalQuarterOf(d: string): { fy: number; q: 1 | 2 | 3 | 4 } {
  const fy = fiscalYearStart(d);
  const m = Number(d.slice(5, 7));
  const q = (m >= 7 && m <= 9 ? 1 : m >= 10 && m <= 12 ? 2 : m >= 1 && m <= 3 ? 3 : 4) as 1 | 2 | 3 | 4;
  return { fy, q };
}
const fqRange = (fy: number, q: 1 | 2 | 3 | 4): DateRange => {
  const starts = [`${fy}-07-01`, `${fy}-10-01`, `${fy + 1}-01-01`, `${fy + 1}-04-01`];
  const ends = [`${fy}-09-30`, `${fy}-12-31`, `${fy + 1}-03-31`, `${fy + 1}-06-30`];
  return { from: starts[q - 1] as string, to: ends[q - 1] as string };
};
...
export function quickRange(key: QuickRangeKey, today: string): DateRange {
  const curMonth = today.slice(0, 7);
  if (key === "monthThis") return { from: `${curMonth}-01`, to: monthEnd(curMonth) };
  if (key === "monthPrev") {
    const m = addMonths(curMonth, -1);
  ...
  // fqPrev
  return q === 1 ? fqRange(fy - 1, 4) : fqRange(fy, ((q - 1) as 1 | 2 | 3));
}